...
{
    top->children.erase(top->children.begin(), top->children.end());
}


ConfigNodeTag ConfigNodeBase::setTag(ConfigNode parent, std::string val)
{
    ConfigNodeTag tag(new ConfigNodeTagData());

    tag->name = val;
    tag->parent =	parent;
    top->children.insert(nodePair(parent,tag));

    return tag;
}

ConfigNodeValue ConfigNodeBase::setValue(ConfigNode parent, std::string val)
{
    ConfigNodeValue value(new ConfigNodeValueData());

    value->value = val;
    value->parent =	parent;
    top->children.insert(nodePair(parent,value));
...
void ConfigNodeBase::dump(std::stringstream& target, uint32_t indent) const
{
    ...
    itrPair ret;
    ret = (top->children).equal_range(stepPointer);//get node children
...
ConfigNode ConfigNodeBase::findNode(const std::string& path) const
{
    ...
    static ConfigNode		targetPointer;
    if(deep_counter == 0)
    {
        targetPointer.reset();
        stepPointer = top;
    ...
    //std::cout<<(bool)targetPointer<<std::endl;
    itrPair ret;
    ret = (top->children).equal_range(stepPointer);															//get node children
...
ConfigNode ConfigNodeBase::getNode(const std::string& path) const
{
    ...
    if(tag)
        return tag;		//поставить здесь проверку на исключения
    else
